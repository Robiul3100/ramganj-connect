    ...
    <div className="min-h-screen bg-background max-w-4xl mx-auto pb-20">
      ...
      <div className="px-4 -mt-2 space-y-4">
        ...
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="অফিসের নাম বা নম্বর খুঁজুন..."
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
          ...
          {categories.map((cat) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap transition-colors ${
                cat === selectedCat ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground"
              }`}
            >
              {cat}
            </button>
                ...
                <div className="p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {office.designation && (
                        <p className="text-xs text-muted-foreground">{office.designation}</p>
                      )}
                      <span className="inline-block mt-1 text-[10px] font-semibold px-2 py-0.5 rounded-full bg-secondary text-secondary-foreground">
                        {office.category}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="mt-3 space-y-1.5">
                    {office.address && (
                      <p className="flex items-center gap-2 text-xs text-muted-foreground">
                        <MapPin className="w-3.5 h-3.5 shrink-0" /> {office.address}
                      </p>
                    )}
                    {office.visiting_hours && (
                      <p className="flex items-center gap-2 text-xs text-muted-foreground">
                        <Clock className="w-3.5 h-3.5 shrink-0" /> {office.visiting_hours}
                      </p>
                    )}
                    {office.description && (
                  ...
                  {office.phone && (
                    <a
                      ...
                      className="mt-3 flex items-center justify-center gap-2 w-full py-2.5 rounded-xl bg-primary/10 text-primary text-xs font-bold hover:bg-primary/20 transition-colors"
                    >
