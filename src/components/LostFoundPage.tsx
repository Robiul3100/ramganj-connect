...
import { Search, MapPin, Calendar, Phone, AlertCircle, HandHeart, User, Gift, Clock, Tag, Eye, Flame, ExternalLink, BadgeCheck, X } from "lucide-react";
...
const lostColors = { accent: "hsl(0,65%,50%)", bg: "hsl(0,65%,94%)", gradient: "linear-gradient(135deg, hsl(0,65%,50%), hsl(15,70%,55%))" };
const foundColors = { accent: "hsl(150,60%,40%)", bg: "hsl(150,60%,94%)", gradient: "linear-gradient(135deg, hsl(150,60%,40%), hsl(160,55%,48%))" };
...
// ──── Lost & Found Card (Doctor Card Style) ────
const LostFoundCard = ({ item }: { item: LostFoundItem }) => {
  const [expanded, setExpanded] = useState(false);
  const isLost = item.type === "lost";
  const colors = isLost ? lostColors : foundColors;
  const isRecent = differenceInHours(new Date(), new Date(item.created_at)) < 24;

  return (
    <div
      className="relative rounded-[16px] bg-card overflow-hidden border border-border/50 transition-all duration-300 hover:shadow-xl group"
    >
      ...
      <div className="relative w-full aspect-video overflow-hidden bg-muted">
        {item.image_url ? (
          ...
          <div className="w-full h-full flex items-center justify-center" style={{ background: colors.bg }}>
            {isLost ? <AlertCircle className="w-16 h-16 opacity-30" style={{ color: colors.accent }} /> : <HandHeart className="w-16 h-16 opacity-30" style={{ color: colors.accent }} />}
          </div>
        )}
        ...
        <div className="absolute top-3 right-0 flex items-center gap-1 px-3 py-1.5 rounded-l-full shadow-lg z-10"
          style={{ background: colors.gradient }}>
          {isLost ? <AlertCircle className="w-3.5 h-3.5 text-white" /> : <HandHeart className="w-3.5 h-3.5 text-white" />}
        ...
        <div className="absolute top-3 left-3 flex items-center gap-1.5 z-10">
          {item.is_high_priority && (
            <span className="px-2.5 py-1 rounded-full text-[9px] font-bold shadow-md flex items-center gap-1"
        ...
        {item.reward && (
          <div className="absolute bottom-3 left-3 flex items-center gap-1.5 px-2.5 py-1 rounded-full shadow-md z-10"
            style={{ background: "linear-gradient(135deg, hsl(45,90%,50%), hsl(35,85%,55%))", color: "white" }}>
            <Gift className="w-3 h-3" />
            <span className="text-[9px] font-bold">পুরষ্কার আছে</span>
          </div>
        )}
      </div>

      {/* B) Primary Info Section */}
      <div className="p-4 sm:p-5">
        ...
        <div className="flex items-center gap-2 mt-1">
          {item.category && item.category !== "সাধারণ" && (
            <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full" style={{ background: colors.bg, color: colors.accent }}>
              {item.category}
            </span>
          )}
          {item.is_verified && (
        ...
        {/* C) Key Details - Icon-based 2-column grid */}
        ...
        {/* D) Person Info Section */}
        <div className="flex items-center gap-3 mt-3 pt-3 border-t border-border/30">
          {item.person_image_url ? (
            <img src={item.person_image_url} alt={item.person_name || ""} className="w-9 h-9 rounded-full object-cover shrink-0 shadow-sm"
              style={{ border: `2.5px solid ${colors.accent}30` }} />
          ) : (
            <div className="w-9 h-9 rounded-full flex items-center justify-center shrink-0 shadow-sm" style={{ background: colors.bg }}>
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[12px] font-bold text-foreground leading-tight mt-0.5 truncate">{item.person_name || "বেনামী"}</p>
          </div>
        ...
        {expanded && (item.detail_description || item.reward) && (
          <div className="mt-3 pt-3 border-t border-border/30 animate-in fade-in slide-in-from-top-2 duration-200 space-y-2">
            ...
            {item.reward && (
              <div className="flex items-start gap-2 rounded-xl p-3" style={{ background: "hsl(45,90%,50%,0.08)" }}>
...
const LostFoundPage = () => {
  ...
  return (
    <div className="min-h-screen bg-background max-w-4xl mx-auto pb-20">
      <PageHeader title="হারানো ও পাওয়া" color={lostColors.gradient} onAdd={() => setShowForm(true)} />
      ...
      <div className="px-4 -mt-2 space-y-3">
        ...
        {loading ? (
          ...
          <>
            <p className="text-xs text-muted-foreground font-semibold">{filtered.length}টি বিজ্ঞপ্তি পাওয়া গেছে</p>
      ...
      <SubmitFormDialog
        ...
        headerColor={lostColors.gradient}
      />
      <BottomNav />
    </div>
  );
